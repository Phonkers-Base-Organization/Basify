...
var __publicField = (obj, key, value) => __defNormalProp(obj, typeof key !== "symbol" ? key + "" : key, value);
  ...
  const _LocalStorageManager = class _LocalStorageManager {
    static get storage() {
    ...
    static loadData() {
      try {
        const rawData = _LocalStorageManager.storage.get(_LocalStorageManager.rootKey);
        if (!rawData) {
          return _LocalStorageManager.createDefaultData();
        }
        const parsedData = JSON.parse(rawData);
        const defaultData = _LocalStorageManager.createDefaultData();
        return {
          ...defaultData,
          ...parsedData,
          settings: {
    ...
    static saveData(data) {
      try {
        _LocalStorageManager.storage.set(_LocalStorageManager.rootKey, JSON.stringify(data));
      } catch (error) {
        console.error("Failed to save extension storage:", error);
      }
    }
    static async updateData(callback) {
      _LocalStorageManager.lock = _LocalStorageManager.lock.then(async () => {
        const data = _LocalStorageManager.loadData();
        await callback(data);
        _LocalStorageManager.saveData(data);
      }).catch((error) => {
        console.error("Storage update failed:", error);
      });
      return _LocalStorageManager.lock;
    }
    static getArtist(id) {
      const data = _LocalStorageManager.loadData();
    ...
    static async saveArtist(artist) {
      ...
      await _LocalStorageManager.updateData((data) => {
        data.artistsById[artist.id] = savedArtist;
        _LocalStorageManager.trimArtistCache(data);
      });
      return savedArtist;
    }
    ...
    static async resetSettings() {
      const defaultSettings = _LocalStorageManager.createDefaultData().settings;
      await _LocalStorageManager.updateData((data) => {
        data.settings = defaultSettings;
        _LocalStorageManager.trimArtistCache(data);
      });
      return _LocalStorageManager.getSettings();
    }
    static clearAll() {
      _LocalStorageManager.storage.remove(_LocalStorageManager.rootKey);
    }
  };
  __publicField(_LocalStorageManager, "rootKey", "Basify:data");
  __publicField(_LocalStorageManager, "lock", Promise.resolve());
  let LocalStorageManager = _LocalStorageManager;
  const BasifyLocales = {
    en: {
  ...
  const _BasifyI18n = class _BasifyI18n {
    static getSpotifyLocale() {
      var _a;
      try {
        return ((_a = Spicetify == null ? void 0 : Spicetify.Locale) == null ? void 0 : _a.getLocale()) || "en";
      } catch (e) {
        return "en";
      }
    }
    static normalizeLocale(locale) {
      ...
      return languageCode === "uk" || languageCode === "ru" ? "uk" : _BasifyI18n.defaultLocale;
    }
    static getInitialLocale() {
      return _BasifyI18n.normalizeLocale(_BasifyI18n.getSpotifyLocale());
    }
    static getLocale() {
      try {
        const settings = LocalStorageManager.getSettings();
        return BasifyLocales[settings.locale] ? settings.locale : _BasifyI18n.defaultLocale;
      } catch (e) {
        return _BasifyI18n.defaultLocale;
      }
    }
    static getPhonkersbaseLocalePath(locale = _BasifyI18n.getLocale()) {
      return locale === "uk" ? "uk" : "en";
    }
    static t(key, locale = _BasifyI18n.getLocale()) {
      if (!key) return "";
      const dictionary = BasifyLocales[locale] || BasifyLocales[_BasifyI18n.defaultLocale];
      return _BasifyI18n.getNestedValue(dictionary, key) ?? _BasifyI18n.getNestedValue(BasifyLocales[_BasifyI18n.defaultLocale], key) ?? key;
    }
    static getNestedValue(object, path) {
      return String(path).split(".").reduce((currentValue, keyPart) => currentValue == null ? void 0 : currentValue[keyPart], object);
    }
    static countryName(countryCode, locale = _BasifyI18n.getLocale()) {
      var _a, _b;
      if (!countryCode) return "";
      const normalizedCountryCode = String(countryCode).trim().toLowerCase();
      const customCountryName = ((_a = _BasifyI18n.lowercaseCountryNames[normalizedCountryCode]) == null ? void 0 : _a[locale]) ?? ((_b = _BasifyI18n.lowercaseCountryNames[normalizedCountryCode]) == null ? void 0 : _b[_BasifyI18n.defaultLocale]);
  ...
  __publicField(_BasifyI18n, "defaultLocale", "en");
  __publicField(_BasifyI18n, "lowercaseCountryNames", {
    "gb-sct": { en: "Scotland", uk: "Шотландія" },
    ru: { en: "russia", uk: "росія" },
    by: { en: "belarus", uk: "білорусь" }
  });
  let BasifyI18n = _BasifyI18n;
  const crownSvg = `<svg width="20" height="20" viewBox="0 0 16 17" style="margin-left:6px; box-sizing:content-box;" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M8 4.5L10.6667 8.5L14 5.83333L12.6667 12.5H3.33333L2 5.83333L5.33333 8.5L8 4.5Z" fill="currentColor" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
  ...
  const _ArtistInfoSectionRenderer = class _ArtistInfoSectionRenderer {
      ...
      if (artist.countries.length) {
        artist.countries.forEach((country) => {
          countriesRow.appendChild(
            _ArtistInfoSectionRenderer.createCountryBadge(country)
          );
        });
      } else {
        countriesRow.appendChild(
          _ArtistInfoSectionRenderer.createSimpleTag(
      ...
      if (artist.labels.length) {
        artist.labels.forEach((label) => {
          badgesRow.appendChild(
            _ArtistInfoSectionRenderer.createTrustBadge(label, artist)
          );
        });
      } else {
        badgesRow.appendChild(
          _ArtistInfoSectionRenderer.createTrustBadge("noInfo", artist)
        );
      }
      section.appendChild(countriesRow);
      section.appendChild(_ArtistInfoSectionRenderer.createInfoSeparator());
    ...
    static createTrustBadge(type, artist = null) {
      const badgeData = _ArtistInfoSectionRenderer.badges[type] ?? _ArtistInfoSectionRenderer.badges.noInfo;
      const badge = document.createElement("span");
  ...
  __publicField(_ArtistInfoSectionRenderer, "badges", {
  ...
  let ArtistInfoSectionRenderer = _ArtistInfoSectionRenderer;
  const _SkipToastRenderer = class _SkipToastRenderer {
    static async bufferTrack(track) {
      _SkipToastRenderer.skippedTracksBuffer.push(track);
      if (_SkipToastRenderer.flushTimeoutId) {
        clearTimeout(_SkipToastRenderer.flushTimeoutId);
      }
      _SkipToastRenderer.flushTimeoutId = setTimeout(() => {
        _SkipToastRenderer.flushBuffer();
      }, 1200);
    }
    static async flushBuffer() {
      const tracks = [..._SkipToastRenderer.skippedTracksBuffer];
      _SkipToastRenderer.skippedTracksBuffer = [];
      _SkipToastRenderer.flushTimeoutId = null;
      if (tracks.length === 0) return;
      const settings = LocalStorageManager.getSettings();
      if (!settings.popupEnabled) return;
      _SkipToastRenderer.injectStyles();
      const lastTrack = tracks[tracks.length - 1];
      const dominantColor = await lastTrack.getDominantColor();
      const container = _SkipToastRenderer.createContainer();
      let toast;
      if (tracks.length === 1) {
        toast = _SkipToastRenderer.createToast(lastTrack, dominantColor);
      } else {
        toast = _SkipToastRenderer.createMultiToast(tracks, dominantColor);
      }
      toast.dataset.createdAt = String(Date.now());
      _SkipToastRenderer.removeExtraToastsBeforeAppend(container);
      container.appendChild(toast);
      _SkipToastRenderer.updateToastStack(container);
      requestAnimationFrame(() => {
        toast.classList.add("is-visible");
      });
      _SkipToastRenderer.scheduleToastRemoval(toast);
    }
    static scheduleToastRemoval(toast) {
      const existingTimeoutId = _SkipToastRenderer.toastTimeouts.get(toast);
      if (existingTimeoutId) {
        clearTimeout(existingTimeoutId);
      }
      const durationMs = _SkipToastRenderer.getToastDurationMs();
      const createdAt = Number(toast.dataset.createdAt) || Date.now();
      const ageMs = Date.now() - createdAt;
      const remainingMs = Math.max(0, durationMs - ageMs);
      const timeoutId = setTimeout(() => {
        _SkipToastRenderer.removeToast(toast);
      }, remainingMs);
      _SkipToastRenderer.toastTimeouts.set(toast, timeoutId);
    }
    static applySettings(settings, changedSettings = {}) {
      if (!settings.popupEnabled) {
        _SkipToastRenderer.clearAll();
        return;
      }
      const container = document.querySelector(`.${_SkipToastRenderer.containerClassName}`);
      if (!container) return;
    ...
    static removeExtraToastsBeforeAppend(container) {
      const maxToastCount = _SkipToastRenderer.getVisibleToastLimit() + 1;
      const toasts = Array.from(container.querySelectorAll(".basify-skip-toast"));
      while (toasts.length >= maxToastCount) {
        const oldestToast = toasts.shift();
        _SkipToastRenderer.removeToast(oldestToast, false);
      }
    }
    static updateToastStack(container) {
      const visibleToastLimit = _SkipToastRenderer.getVisibleToastLimit();
      const toasts = Array.from(container.querySelectorAll(".basify-skip-toast"));
      toasts.forEach((toast) => {
    ...
    static createContainer() {
      let container = document.querySelector(`.${_SkipToastRenderer.containerClassName}`);
      if (container) return container;
      container = document.createElement("div");
      container.className = _SkipToastRenderer.containerClassName;
    ...
    static createToast(track, dominantColor) {
      ...
      _SkipToastRenderer.applyDominantColorBackground(toast, dominantColor);
      ...
      skipReasons.forEach((reason) => {
        if (reason.type === "distributor") {
          artistsWrapper.appendChild(
            _SkipToastRenderer.createDistributorReasonRow(reason.name, reason.label)
          );
          return;
        }
        artistsWrapper.appendChild(
          _SkipToastRenderer.createArtistReasonRow(reason.artist, [reason.label])
        );
      });
    ...
    static createArtistReasonRow(artist, labels) {
      ...
      artistName.textContent = (artist == null ? void 0 : artist.name) || BasifyI18n.t("unknownArtist");
      if (artist == null ? void 0 : artist.id) {
    ...
    static removeToast(toast, animate = true) {
      ...
      const timeoutId = _SkipToastRenderer.toastTimeouts.get(toast);
      if (timeoutId) {
        clearTimeout(timeoutId);
        _SkipToastRenderer.toastTimeouts.delete(toast);
      }
      const container = toast.closest(`.${_SkipToastRenderer.containerClassName}`);
      if (!animate) {
        toast.remove();
        if (container == null ? void 0 : container.children.length) {
          _SkipToastRenderer.updateToastStack(container);
        } else {
    ...
    static clearAll() {
      ...
      container.querySelectorAll(".basify-skip-toast").forEach((toast) => {
        _SkipToastRenderer.removeToast(toast, false);
      });
      container.remove();
    }
    static injectStyles() {
      if (document.getElementById(_SkipToastRenderer.styleElementId)) return;
      const style = document.createElement("style");
      style.id = _SkipToastRenderer.styleElementId;
      style.textContent = `
  ...
  __publicField(_SkipToastRenderer, "styleElementId", "basify-skip-toast-style");
  __publicField(_SkipToastRenderer, "containerClassName", "basify-skip-toast-container");
  __publicField(_SkipToastRenderer, "toastTimeouts", /* @__PURE__ */ new WeakMap());
  __publicField(_SkipToastRenderer, "skippedTracksBuffer", []);
  __publicField(_SkipToastRenderer, "flushTimeoutId", null);
  let SkipToastRenderer = _SkipToastRenderer;
  const _NowPlayingRuntimeState = class _NowPlayingRuntimeState {
    static update(track, artistSpans = null) {
      _NowPlayingRuntimeState.track = track;
      _NowPlayingRuntimeState.artistSpans = artistSpans;
    }
  };
  __publicField(_NowPlayingRuntimeState, "track", null);
  __publicField(_NowPlayingRuntimeState, "artistSpans", null);
  let NowPlayingRuntimeState = _NowPlayingRuntimeState;
  const _NowPlayingThemeOverlayRenderer = class _NowPlayingThemeOverlayRenderer {
    static applyFromTrack(track) {
      const settings = LocalStorageManager.getSettings();
      if (!settings.formatNowPlayingBar) {
        _NowPlayingThemeOverlayRenderer.clear();
        return;
      }
      ...
      _NowPlayingThemeOverlayRenderer.apply(themeStatus);
    }
    static apply(themeStatus) {
      const nowPlayingBar = _NowPlayingThemeOverlayRenderer.getNowPlayingBar();
      const themeColor = ArtistInfoSectionRenderer.badges[themeStatus].bg;
      if (!nowPlayingBar || !themeColor) return;
      _NowPlayingThemeOverlayRenderer.injectStyles();
      let overlay = nowPlayingBar.querySelector(`#${_NowPlayingThemeOverlayRenderer.overlayElementId}`);
      if (!overlay) {
        overlay = document.createElement("div");
        overlay.id = _NowPlayingThemeOverlayRenderer.overlayElementId;
    ...
    static clear() {
      var _a;
      (_a = document.getElementById(_NowPlayingThemeOverlayRenderer.overlayElementId)) == null ? void 0 : _a.remove();
    }
    static getNowPlayingBar() {
      return document.querySelector("div.Root__now-playing-bar");
    }
    static injectStyles() {
      if (document.getElementById(_NowPlayingThemeOverlayRenderer.styleElementId)) return;
      const style = document.createElement("style");
      style.id = _NowPlayingThemeOverlayRenderer.styleElementId;
      style.textContent = `
  ...
  __publicField(_NowPlayingThemeOverlayRenderer, "styleElementId", "basify-now-playing-theme-overlay-style");
  __publicField(_NowPlayingThemeOverlayRenderer, "overlayElementId", "basify-now-playing-theme-overlay");
  let NowPlayingThemeOverlayRenderer = _NowPlayingThemeOverlayRenderer;
  class Country {
  ...
  const _Artist = class _Artist {
    ...
    static async create(artistId, fallbackName = null) {
      ...
      if (cachedArtistData) {
        ...
        return new _Artist(updatedArtistData || cachedArtistData);
      }
      const fetchedArtistData = await _Artist.fetch(artistId, fallbackName);
      const savedArtistData = await LocalStorageManager.saveArtist(fetchedArtistData);
      return new _Artist(savedArtistData);
    }
    static async fetch(artistId, fallbackName = null) {
      var _a, _b;
      const artistURL = _Artist.baseArtistURL + artistId;
      const requestURL = _Artist.apiURL + encodeURIComponent(artistURL);
      try {
        const responseData = await Spicetify.CosmosAsync.get(requestURL);
        const artistItem = (_b = (_a = responseData == null ? void 0 : responseData.data) == null ? void 0 : _a.items) == null ? void 0 : _b[0];
        if (!artistItem) {
          return {
            id: artistId,
            name: fallbackName,
        ...
        return {
          ...
          countries: (artistItem.countries || []).map((countryData) => {
            var _a2;
            const countryInfo = ((_a2 = countryData.originalName) == null ? void 0 : _a2.split(" ")) || [];
  ...
  __publicField(_Artist, "baseArtistURL", "open.spotify.com/artist/");
  __publicField(_Artist, "apiURL", "https://www.phonkersbase.com/api/artists?limit=50&offset=0&locale=en&search=");
  let Artist = _Artist;
  const _NowPlayingArtistRenderer = class _NowPlayingArtistRenderer {
    static render(basifyTrack, artistSpans) {
      const settings = LocalStorageManager.getSettings();
      _NowPlayingArtistRenderer.injectStyles();
      _NowPlayingArtistRenderer.renderArtistSpanGroup(
    ...
    static renderArtistSpanGroup(artistSpansById, artists, settings) {
      Object.entries(artistSpansById).forEach(([artistId, artistSpan]) => {
        ...
        _NowPlayingArtistRenderer.resetArtistSpan(artistSpan);
        if (!artist) return;
        const dominantLabel = _NowPlayingArtistRenderer.getDominantArtistLabel(artist);
        const statusStyle = _NowPlayingArtistRenderer.statusStyles[dominantLabel];
        const artistLink = artistSpan.querySelector("a") || artistSpan;
        if (settings.formatNowPlayingArtistName && (statusStyle == null ? void 0 : statusStyle.color)) {
        ...
        if (settings.showNowPlayingArtistStatusShape && statusStyle) {
          artistSpan.appendChild(_NowPlayingArtistRenderer.createStatusShape(dominantLabel));
        }
    ...
    static getDominantArtistLabel(artist) {
      ...
      return _NowPlayingArtistRenderer.labelPriority.find((label) => labels.includes(label)) || "noInfo";
    }
    static createStatusShape(label) {
      const statusStyle = _NowPlayingArtistRenderer.statusStyles[label] || _NowPlayingArtistRenderer.statusStyles.noInfo;
      ...
      const iconSvg = _NowPlayingArtistRenderer.statusIcons[statusStyle.shape];
      if (iconSvg) {
    ...
    static injectStyles() {
      if (document.getElementById(_NowPlayingArtistRenderer.styleElementId)) return;
      const style = document.createElement("style");
      style.id = _NowPlayingArtistRenderer.styleElementId;
      style.textContent = `
  ...
  __publicField(_NowPlayingArtistRenderer, "styleElementId", "basify-now-playing-artist-style");
  __publicField(_NowPlayingArtistRenderer, "labelPriority", ["blocked", "warning", "unknown", "pride", "base", "approved", "noInfo"]);
  __publicField(_NowPlayingArtistRenderer, "statusStyles", {
  ...
  __publicField(_NowPlayingArtistRenderer, "statusIcons", {
    crown: crownSvg,
    star: starSvg,
    world: unknownSvg
  });
  let NowPlayingArtistRenderer = _NowPlayingArtistRenderer;
  class DomObserver {
    ...
    static async waitForNowPlayingArtist(track, timeoutMs = 5e3) {
      return DomObserver.waitUntil(() => {
        ...
        const trackArtistIds = ((track == null ? void 0 : track.artists) || []).map((artist) => artist.uri.split(":")[2]);
  ...
  const _PlaylistViewRenderer = class _PlaylistViewRenderer {
    static start() {
      _PlaylistViewRenderer.injectStyles();
      _PlaylistViewRenderer.registerGlobalInterceptors();
      if (_PlaylistViewRenderer.observer) {
        _PlaylistViewRenderer.observer.disconnect();
      }
      _PlaylistViewRenderer.observer = new MutationObserver(() => {
        _PlaylistViewRenderer.scanRows();
      });
      const mainView = document.querySelector(".main-view-container");
      if (mainView) {
        _PlaylistViewRenderer.observer.observe(mainView, {
          childList: true,
          subtree: true
        });
        _PlaylistViewRenderer.scanRows();
    ...
    static registerGlobalInterceptors() {
      if (_PlaylistViewRenderer.eventsRegistered) return;
      _PlaylistViewRenderer.eventsRegistered = true;
      ...
      try {
        ...
        allTracks.forEach((item) => {
          ...
          (trackData.artists || []).forEach((artist) => {
            var _a, _b, _c;
            const id = (_c = (_b = (_a = artist.uri) == null ? void 0 : _a.split) == null ? void 0 : _b.call(_a, ":")) == null ? void 0 : _c[2];
    ...
    static async renderRatingCard(playlistId) {
      ...
      try {
        ...
        if (!settings.showPlaylistRating) {
          (_a = document.getElementById("basify-playlist-rating-card")) == null ? void 0 : _a.remove();
          return;
        }
        console.log(`[Basify] Attempting to render rating card for: ${playlistId}`);
        const rating = await _PlaylistViewRenderer.calculatePlaylistRating(playlistId);
        if (!rating) return;
        ...
        (_b = document.getElementById("basify-playlist-rating-card")) == null ? void 0 : _b.remove();
      ...
      `;
        ...
        requestAnimationFrame(() => {
          ...
          if (fillPath && percentageText) {
            ...
            _PlaylistViewRenderer.animateCounter(percentageText, rating.percentage, 800);
          }
        });
    ...
    static async scanRows() {
      ...
      rows.forEach(async (row) => {
        ...
        row.querySelectorAll(".basify-playlist-status-icon-wrapper").forEach((el) => {
          el.removeEventListener("mouseenter", _PlaylistViewRenderer.showTooltip);
          el.removeEventListener("mouseleave", _PlaylistViewRenderer.hideTooltip);
          el.remove();
        });
        ...
        try {
          ...
          artists.forEach((artist, index) => {
            ...
            if (dominantLabel === "blocked" || dominantLabel === "warning" || dominantLabel === "unknown") {
              ...
              wrapper.addEventListener("mouseenter", _PlaylistViewRenderer.showTooltip);
              wrapper.addEventListener("mouseleave", _PlaylistViewRenderer.hideTooltip);
    ...
    static injectStyles() {
      if (document.getElementById(_PlaylistViewRenderer.styleElementId)) return;
      const style = document.createElement("style");
      style.id = _PlaylistViewRenderer.styleElementId;
      style.textContent = `
      .basify-blocked-row {
        opacity: 0.35;
  ...
  __publicField(_PlaylistViewRenderer, "styleElementId", "basify-playlist-view-style");
  __publicField(_PlaylistViewRenderer, "observer", null);
  __publicField(_PlaylistViewRenderer, "eventsRegistered", false);
  let PlaylistViewRenderer = _PlaylistViewRenderer;
  Spicetify.React;
  const _SettingsMenu = class _SettingsMenu {
    static registerButton() {
      if (_SettingsMenu.button) return;
      _SettingsMenu.injectStyles();
      _SettingsMenu.button = new Spicetify.Topbar.Button(
        BasifyI18n.t("settingsTitle"),
        _SettingsMenu.icons.settings,
        _SettingsMenu.open,
        false,
        false
      );
      _SettingsMenu.applyTopbarButtonClass();
    }
    static applyTopbarButtonClass() {
      requestAnimationFrame(() => {
        var _a;
        const buttonElement = (_a = _SettingsMenu.button) == null ? void 0 : _a.element;
    ...
    static updateLocalizedTitles(locale = BasifyI18n.getLocale()) {
      _SettingsMenu.updateButtonTitle(locale);
      _SettingsMenu.updateModalTitle(locale);
    }
    static updateButtonTitle(locale = BasifyI18n.getLocale()) {
      var _a, _b, _c, _d, _e, _f;
      const title = BasifyI18n.t("settingsTitle", locale);
      if (!_SettingsMenu.button) return;
      _SettingsMenu.button.label = title;
      (_a = _SettingsMenu.button.element) == null ? void 0 : _a.setAttribute("aria-label", title);
      (_b = _SettingsMenu.button.element) == null ? void 0 : _b.setAttribute("title", title);
      (_d = (_c = _SettingsMenu.button.tippy) == null ? void 0 : _c.setContent) == null ? void 0 : _d.call(_c, title);
      (_f = (_e = _SettingsMenu.button.tippy) == null ? void 0 : _e.setProps) == null ? void 0 : _f.call(_e, { content: title });
    }
    ...
    static open() {
      ...
      ReactDOM.render(React.createElement(_SettingsMenu.Component), container);
    }
    static async applyRuntimeSettings(settings, changedSettings = {}) {
      ...
      if (localeChanged) {
        _SettingsMenu.updateLocalizedTitles(settings.locale);
      }
      const track = NowPlayingRuntimeState.track;
      const nowPlayingSettingsChanged = Object.hasOwn(changedSettings, "formatNowPlayingBar") || Object.hasOwn(changedSettings, "formatNowPlayingArtistName") || Object.hasOwn(changedSettings, "showNowPlayingArtistStatusShape") || Object.hasOwn(changedSettings, "showNowPlayingArtistFlags") || emojiFlagsChanged;
      ...
      const skipSettingsChanged = Object.hasOwn(changedSettings, "skipEnabled") || Object.hasOwn(changedSettings, "skipBlockedArtists") || Object.hasOwn(changedSettings, "skipWarningArtists") || Object.hasOwn(changedSettings, "skipUnknownArtists");
    ...
    static Component() {
      ...
      const saveSettings = async (newSettings) => {
        ...
        _SettingsMenu.applyRuntimeSettings(updatedSettings, newSettings).catch((error) => {
      ...
      const resetSettings = async () => {
        ...
        _SettingsMenu.applyRuntimeSettings(defaultSettings, defaultSettings).catch((error) => {
      ...
      return React.createElement(
        ...
        React.createElement(_SettingsMenu.SectionTitle, { title: t("language") }),
        React.createElement(_SettingsMenu.SelectRow, {
          label: t("locale"),
        ...
        React.createElement(_SettingsMenu.SectionTitle, { title: t("skipping") }),
        React.createElement(_SettingsMenu.ToggleRow, {
          label: t("skipTracks"),
        ...
        React.createElement(_SettingsMenu.SubSectionTitle, { title: t("skipStatusFilter") }),
        React.createElement(_SettingsMenu.ToggleRow, {
          label: t("blocked"),
        ...
        React.createElement(_SettingsMenu.SectionTitle, { title: t("popup") }),
        React.createElement(_SettingsMenu.ToggleRow, {
        ...
        React.createElement(_SettingsMenu.NumberRow, {
        ...
        React.createElement(_SettingsMenu.SectionTitle, { title: t("flags") }),
        React.createElement(_SettingsMenu.ToggleRow, {
        ...
        React.createElement(_SettingsMenu.SectionTitle, { title: t("nowPlaying") }),
        React.createElement(_SettingsMenu.ToggleRow, {
        ...
        React.createElement(_SettingsMenu.SectionTitle, { title: t("playlistRating") }),
        React.createElement(_SettingsMenu.ToggleRow, {
        ...
        React.createElement(_SettingsMenu.SectionTitle, { title: t("storage") }),
        React.createElement(_SettingsMenu.NumberRow, {
        ...
        React.createElement(_SettingsMenu.SectionTitle, { title: t("reset") }),
        React.createElement(_SettingsMenu.ButtonRow, {
          label: t("clearBasifyData"),
          description: t("clearBasifyDataDescription"),
          buttonText: t("resetButton"),
          onClick: resetSettings
        }),
        React.createElement(_SettingsMenu.InfoSection, { locale: settings.locale })
      );
    }
    ...
    static ToggleRow({ label, description, value, disabled = false, onChange }) {
      ...
      return Spicetify.React.createElement(
        ...
        Spicetify.React.createElement(_SettingsMenu.RowText, { label, description }),
    ...
    static RowText({ label, description }) {
      return Spicetify.React.createElement(
        ...
        description ? Spicetify.React.createElement("div", { className: "basify-settings-description" }, description) : null
      );
    }
      ...
      return Spicetify.React.createElement(
        ...
        Spicetify.React.createElement(
          ...
          Spicetify.React.createElement("span", {
            ...
            dangerouslySetInnerHTML: { __html: _SettingsMenu.icons.phonkersbaseLogoSvg }
          })
        ),
        Spicetify.React.createElement(
          "div",
    ...
    static injectStyles() {
      if (document.getElementById(_SettingsMenu.styleElementId)) return;
      const style = document.createElement("style");
      style.id = _SettingsMenu.styleElementId;
      style.textContent = `
  ...
  __publicField(_SettingsMenu, "button", null);
  __publicField(_SettingsMenu, "styleElementId", "basify-settings-style");
  __publicField(_SettingsMenu, "icons", {
    settings: settingsSvg,
    phonkersbaseLogoSvg
  });
  let SettingsMenu = _SettingsMenu;
  const _PlaybackDeviceMonitor = class _PlaybackDeviceMonitor {
    static getActiveDevice() {
      return Spicetify.Platform.ConnectAPI.state.activeDevice ?? null;
    }
    static isSpotifyPlayingOnCurrentDevice() {
      const device = _PlaybackDeviceMonitor.getActiveDevice();
      if (!device) return false;
      const isLocal = device.isLocal || device.type.toLowerCase() === "computer";
      return Boolean(isLocal);
    }
    static check(reason = "Device check") {
      _PlaybackDeviceMonitor.getActiveDevice();
      const playingOnCurrentDevice = _PlaybackDeviceMonitor.isSpotifyPlayingOnCurrentDevice();
      if (playingOnCurrentDevice !== _PlaybackDeviceMonitor.lastPlayingOnCurrentDevice) {
        console.log(`[Basify] Device Status Changed: Playing locally = ${playingOnCurrentDevice} (${reason})`);
      }
      _PlaybackDeviceMonitor.lastPlayingOnCurrentDevice = playingOnCurrentDevice;
      if (playingOnCurrentDevice && (reason === "Song changed" || reason === "Startup")) {
        handleCurrentTrackSkipCheck(reason);
      }
    }
    static start() {
      if (_PlaybackDeviceMonitor.intervalId) return;
      console.log("[Basify] Starting PlaybackDeviceMonitor");
      _PlaybackDeviceMonitor.check("Startup");
      _PlaybackDeviceMonitor.intervalId = setInterval(() => {
        _PlaybackDeviceMonitor.check("Interval check");
      }, _PlaybackDeviceMonitor.checkIntervalMs);
    }
  };
  __publicField(_PlaybackDeviceMonitor, "checkIntervalMs", 2e3);
  __publicField(_PlaybackDeviceMonitor, "intervalId", null);
  __publicField(_PlaybackDeviceMonitor, "lastDeviceId", null);
  __publicField(_PlaybackDeviceMonitor, "lastPlayingOnCurrentDevice", null);
  let PlaybackDeviceMonitor = _PlaybackDeviceMonitor;
  ...
  class BasifyTrack {
    constructor(spotifyTrack, trackArtists, distributors = []) {
      ...
      this.id = ((_b = (_a = spotifyTrack.uri) == null ? void 0 : _a.split(":")) == null ? void 0 : _b[2]) || null;
    ...
    static async getDistributorsFromSpotifyTrack(spotifyTrack) {
      var _a, _b, _c;
      const albumUri = (_a = spotifyTrack == null ? void 0 : spotifyTrack.album) == null ? void 0 : _a.uri;
      if (!albumUri) return [];
      try {
        ...
        const album = (_b = response == null ? void 0 : response.data) == null ? void 0 : _b.albumUnion;
        const distributorTexts = [];
        if (album == null ? void 0 : album.label) {
          distributorTexts.push(album.label);
        }
        (((_c = album == null ? void 0 : album.copyright) == null ? void 0 : _c.items) || []).forEach((item) => {
          if (!(item == null ? void 0 : item.text)) return;
    ...
    static getTrackImageUrl(spotifyTrack) {
      ...
      const imageUri = ((_b = (_a = spotifyTrack == null ? void 0 : spotifyTrack.images) == null ? void 0 : _a[0]) == null ? void 0 : _b.url) || ((_e = (_d = (_c = spotifyTrack == null ? void 0 : spotifyTrack.album) == null ? void 0 : _c.images) == null ? void 0 : _d[0]) == null ? void 0 : _e.url) || ((_f = spotifyTrack == null ? void 0 : spotifyTrack.metadata) == null ? void 0 : _f.image_xlarge_url) || ((_g = spotifyTrack == null ? void 0 : spotifyTrack.metadata) == null ? void 0 : _g.image_large_url) || ((_h = spotifyTrack == null ? void 0 : spotifyTrack.metadata) == null ? void 0 : _h.image_url);
      if (!imageUri) return null;
      if (imageUri.startsWith("https://")) return imageUri;
      const imageId = (_i = imageUri.split(":")) == null ? void 0 : _i[2];
  ...
  const _ArtistPageHeaderRenderer = class _ArtistPageHeaderRenderer {
    static apply(artistHeaderElement, artist) {
      var _a;
      _ArtistPageHeaderRenderer.injectStyles();
      artistHeaderElement.style.removeProperty("height");
      artistHeaderElement.classList.add("basify-artist-header-wrapper");
      _ArtistPageHeaderRenderer.resetTitleStyles(artistHeaderElement);
      const headerTextElement = artistHeaderElement.querySelector(".main-entityHeader-headerText");
      if (!headerTextElement) return;
      (_a = artistHeaderElement.querySelector(".basify-artist-info-section")) == null ? void 0 : _a.remove();
    ...
    static injectStyles() {
      if (document.getElementById(_ArtistPageHeaderRenderer.styleElementId)) return;
      const style = document.createElement("style");
      style.id = _ArtistPageHeaderRenderer.styleElementId;
      style.textContent = `
  ...
  __publicField(_ArtistPageHeaderRenderer, "styleElementId", "basify-artist-page-header-style");
  let ArtistPageHeaderRenderer = _ArtistPageHeaderRenderer;
  ...
  async function getTrackArtists(track) {
    const artistsData = (track == null ? void 0 : track.artists) || [];
  ...
  function fastSkipCheck() {
    ...
    if (!(track == null ? void 0 : track.uri) || !((_b = track == null ? void 0 : track.artists) == null ? void 0 : _b.length)) return false;
    const settings = LocalStorageManager.getSettings();
    if (!settings.skipEnabled) return false;
  ...
  async function waitForCurrentSpotifyTrack(timeoutMs = 1e4) {
    return await DomObserver.waitUntil(() => {
      var _a, _b;
      const track = (_a = Spicetify.Player.data) == null ? void 0 : _a.item;
      return (track == null ? void 0 : track.uri) && ((_b = track == null ? void 0 : track.artists) == null ? void 0 : _b.length) ? track : null;
  ...
  function isStillCurrentTrack(trackUri) {
    var _a, _b;
    return ((_b = (_a = Spicetify.Player.data) == null ? void 0 : _a.item) == null ? void 0 : _b.uri) === trackUri;
  }
  ...
  (function init() {
    var _a, _b;
    if (!(Spicetify == null ? void 0 : Spicetify.Player) || !((_b = (_a = Spicetify == null ? void 0 : Spicetify.Platform) == null ? void 0 : _a.ConnectAPI) == null ? void 0 : _b.state) || !(Spicetify == null ? void 0 : Spicetify.Locale)) {
      setTimeout(init, 100);
      return;
    }
    main();
